...
import {ArrowDown,ArrowLeft,ArrowRight,ArrowUpRight,Check,Copy,Instagram,Menu,MessageCircle,X} from 'lucide-react'
...
const featured=[works[44],works[49],works[50],works[16],works[1]]
...
function App(){
  ...
  const visible=useMemo(()=>filter==='Todos'?works:filter==='Outros'?works.filter(w=>w.client!=='Bar do Beco'):works.filter(w=>w.client===filter),[filter])
  ...
  const whatsappMessage=encodeURIComponent('Olá, WS Design! Vi seu portfólio e quero criar um flyer para meu evento.')
  ...
  return <div ref={root} className="site-shell">
    ...
    <header className="nav-shell">
      ...
      <nav className={cn('nav-links',menu&&'nav-open')} aria-label="Principal">
        <button onClick={()=>scroll('trabalhos')}>Trabalhos</button><button onClick={()=>scroll('sobre')}>Sobre</button><button onClick={()=>scroll('contato')}>Contato</button>
      </nav>
    ...
    <main>
      ...
      <section id="trabalhos" className="work section-pad">
        ...
        <div className="gallery">
          {visible.map((work,i)=><motion.button layout key={work.file} className="work-card" initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-5%'}} transition={{duration:.45,delay:Math.min(i%6*.04,.2),ease:'easeOut'}} onClick={()=>setActive(work.id-1)} aria-label={`Abrir ${work.title}`}>
            <div className="image-wrap"><img src={work.src} alt={work.title} loading="lazy"/><span className="card-open"><ArrowUpRight/></span></div><div className="card-meta"><span>{work.client}</span><span>{work.month} · 2026</span></div>
          </motion.button>)}
        </div>
      </section>
      ...
      <section id="sobre" className="about section-pad">
        ...
        <div className="about-copy" data-reveal><span className="index">02</span><p className="kicker">Por trás dos pixels</p><h2>Design com<br/><em>presença.</em></h2><p className="lead">Transformo a energia de cada evento em uma peça visual impossível de ignorar.</p><p>Do conceito à entrega final, cada composição combina hierarquia forte, recortes precisos e tipografia com personalidade — pronta para feed, story e divulgação.</p><div className="stats"><div><strong>50+</strong><span>peças na seleção</span></div><div><strong>100%</strong><span>foco em eventos</span></div><div><strong>01</strong><span>visual memorável</span></div></div></div>
      </section>

      <section id="contato" className="contact section-pad" data-reveal>
        <span className="index">03</span><p className="kicker">Seu próximo evento</p><h2>VAMOS FAZER<br/><em>BARULHO?</em></h2><p>Conte a ideia, a data e o formato. Eu cuido do impacto visual.</p>
